...
def parse_args():
    parser = argparse.ArgumentParser(
        description="""
        Takes a csv with newick strings and calculates
...
def main():
    ...
    table_with_networks = pd.read_csv(input_path, sep=";", header='infer', index_col=False)

    print(table_with_networks)
    ...
    for index, row in table_with_networks.iterrows():
        print(index)
        newick = row["Newick"]
    ...
    network_properties = network_properties.join(table_with_networks)
